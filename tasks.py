...
def my_mlp(w, X, sigma=np.tanh):
  ...
  W3 = w[(4*6)+(7*4):(4*6)+(7*4)+7].reshape(1,7)
  ...
  return f # change this to `return f`
...
# -----------------------------------------------
def dR(beta, x, y):
    dbeta_0 = 2/y.shape[0] * np.sum(beta[0] + beta[1]*x - y) # implement the above formula for dR/dβ₀
    dbeta_1 = 2/y.shape[0] * np.sum((beta[0] + beta[1]*x - y)*x) # implement the above formula for dR/dβ₁
